<template>
  <div>
    <div class="filter-bar">
      ...
      <button 
        class="filter-btn" 
        :class="{ active: !purchaseOnly && minPremium === 0 }" 
      ...
      <button 
        class="filter-btn" 
        :class="{ active: minPremium === 5 && !purchaseOnly }" 
        @click="setFilter(5, false)"
      >溢价>5%</button>
    </div>
...
const fetchData = async () => {
  loading.value = true
  try {
    const data = await api.getQDIIRanking({ 
    ...
    console.error('获取QDII数据失败:', error)
  } finally {
    loading.value = false
  }
}
